function fpeData = process3DFootPlacementEstimator(...
  mass,...
  wholeBodyData,...
  columnCenterOfMassPosition, columnCenterOfMassVelocity,...
  columnInertia,columnAngularMomentum,...
  numberOfEndPointsToIgnore,...
  gravityVector,...
  contactPlanes,...
  solutionTolerance,...
  maximumIterations,...
  flag_fpeEvaluateDerivatives,...
  flag_fpeVerbose,...
  fpeFileName,...
  flag_loadFromFile)



if(flag_loadFromFile==0)
  
  npts = size(wholeBodyData,1);
  nPlanes = size(contactPlanes,1);

  if(flag_fpeEvaluateDerivatives==1)
    fpeData(nPlanes) = ...
          struct('f'               ,  zeros(npts,1),...
                 'phi'             ,  zeros(npts,1),...
                 'r0F0'            ,  zeros(npts,3),...
                 'projectionError' ,  zeros(npts,1),...
                 'n'               ,  zeros(npts,3),...
                 'r0G0'            ,  zeros(npts,3),...
                 'omega'           ,  zeros(npts,3),...
                 'h'               ,  zeros(npts,1),...
                 'vx'              ,  zeros(npts,1),...
                 'vy'              ,  zeros(npts,1),...
                 'J'               ,  zeros(npts,1),...                 
                 'Df_Dphi'         ,  zeros(npts,1),...
                 'Df_Domega'       ,  zeros(npts,1),...
                 'Df_Dh'           ,  zeros(npts,1),...
                 'Df_Dvx'          ,  zeros(npts,1),...
                 'Df_Dvy'          ,  zeros(npts,1),...
                 'Df_DJ'           ,  zeros(npts,1),...
                 'Df_Dm'           ,  zeros(npts,1),...
                 'Df_Dg'           ,  zeros(npts,1));
  else
    fpeData(nPlanes) = ...
          struct('f'               ,  zeros(npts,1),...
                 'phi'             ,  zeros(npts,1),...
                 'r0F0'            ,  zeros(npts,3),...
                 'projectionError' ,  zeros(npts,1),...
                 'n'               ,  zeros(npts,3),...
                 'r0G0'            ,  zeros(npts,3),...
                 'omega'           ,  zeros(npts,3),...
                 'h'               ,  zeros(npts,1),...
                 'vx'              ,  zeros(npts,1),...
                 'vy'              ,  zeros(npts,1),...
                 'J'               ,  zeros(npts,1));

  end

  for i=numberOfEndPointsToIgnore:1:(npts-numberOfEndPointsToIgnore)
    r0C0 = wholeBodyData(i,columnCenterOfMassPosition)';
    v0C0 = wholeBodyData(i,columnCenterOfMassVelocity)';
    JC0 = [wholeBodyData(i,columnInertia(1,1:3));...
         wholeBodyData(i,columnInertia(1,4:6));...
         wholeBodyData(i,columnInertia(1,7:9))];
    HC0 = [wholeBodyData(i,columnAngularMomentum(1,1:3))]';    

    for j=1:1:size(contactPlanes,1)

      fpeInfo = calc3DFootPlacementEstimatorInfo(mass,...
                                              r0C0,...
                                              v0C0,...                                                    
                                              JC0,...                                                    
                                              HC0,...
                                              contactPlanes(j,:)',...
                                              gravityVector,...
                                              solutionTolerance,...
                                              maximumIterations,...
                                              flag_fpeEvaluateDerivatives,...
                                              flag_fpeVerbose);

      fpeData(j).f(i,1)               = fpeInfo.f;
      fpeData(j).phi(i,1)             = fpeInfo.phi;
      fpeData(j).r0F0(i,:)            = fpeInfo.r0F0';
      fpeData(j).projectionError(i,:) = fpeInfo.projectionError;
      fpeData(j).n(i,:)               = fpeInfo.n';
      fpeData(j).r0G0(i,:)            = fpeInfo.r0G0';
      fpeData(j).omega(i,:)           = fpeInfo.omega;
      fpeData(j).h(i,:)               = fpeInfo.h ;
      fpeData(j).vx(i,:)              = fpeInfo.vx;
      fpeData(j).vy(i,:)              = fpeInfo.vy;
      fpeData(j).J(i,:)               = fpeInfo.J;



      if(flag_fpeEvaluateDerivatives==1)
        fpeData(j).Df_Dphi(i,1)    = fpeInfo.Df_Dphi  ;  
        fpeData(j).Df_Domega(i,1)  = fpeInfo.Df_Domega;  
        fpeData(j).Df_Dh(i,1)      = fpeInfo.Df_Dh    ;  
        fpeData(j).Df_Dvx(i,1)     = fpeInfo.Df_Dvx   ;  
        fpeData(j).Df_Dvy(i,1)     = fpeInfo.Df_Dvy   ;  
        fpeData(j).Df_DJ(i,1)     = fpeInfo.Df_DJ    ;           
        fpeData(j).Df_Dm(i,1)      = fpeInfo.Df_Dm    ;           
        fpeData(j).Df_Dg(i,1)      = fpeInfo.Df_Dg    ;           
      end
    end
  end  
  

  
  save(fpeFileName,...
      'fpeData');    
else
  load( fpeFileName);
end
