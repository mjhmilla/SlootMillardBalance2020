...
if(c3dGrfDataAvailable ==1)
  ...
  cornerErr1 = norm(c3dForcePlates(1).corners...
                   -fpAtIndex1ErrorStruct.corners');
  fpCopOffset(2) = struct('xy',[]);
                                     
  if(cornerErr1 > 1)
    fpCopOffset(1).xy = fpAtIndex2ErrorStruct.error;
    fpCopOffset(2).xy = fpAtIndex1ErrorStruct.error;   
    
    assert(norm(c3dForcePlates(1).corners...
                   -fpAtIndex2ErrorStruct.corners') < 1);    
    assert(norm(c3dForcePlates(2).corners...
                   -fpAtIndex1ErrorStruct.corners') < 1);
    
  else
    fpCopOffset(1).xy = fpAtIndex1ErrorStruct.error;
    fpCopOffset(2).xy = fpAtIndex2ErrorStruct.error;
  ...
  for i=1:1:length(c3dForcePlates)
    ...
    for j=1:1:numberOfItems
      ...
      c3dGrf(i).cop(j,:)    = (p + [dx,dy,dz]).*scaleDistance ...
             - [fpCopOffset(i).xy(1,1),fpCopOffset(i).xy(1,2),0].*scaleDistance;
